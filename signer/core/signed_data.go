...
var (
	...
	ApplicationThemis = SigFormat{
		accounts.MimetypeThemis,
		0x03,
	}
	TextPlain = SigFormat{
...
func (api *SignerAPI) determineSignatureFormat(ctx context.Context, contentType string, addr common.MixedcaseAddress, data interface{}) (*SignDataRequest, bool, error) {
	...
	case ApplicationThemis.Mime:
		...
		header := &types.Header{}
		if err := rlp.DecodeBytes(themisData, header); err != nil {
			return nil, useEthereumV, err
		}
		// The incoming clique header is already truncated, sent to us with a extradata already shortened
		if len(header.Extra) < 65 {
		...
		messages := []*NameValueType{
			{
				Name:  "Themis header",
				Typ:   "themis",
				Value: fmt.Sprintf("themis header %d [0x%x]", header.Number, header.Hash()),
			},
		}
		// Clique uses V on the form 0 or 1
		useEthereumV = false
		req = &SignDataRequest{ContentType: mediaType, Rawdata: themisRlp, Messages: messages, Hash: sighash}
